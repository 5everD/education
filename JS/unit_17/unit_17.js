...
function t10() {
    let a10_res = [];
    a10.forEach(elem => {
        if (elem < 0) a10_res.push(0);
        else a10_res.push(elem);
    });
    a10 = a10_res
    return a10;
}
...
let a14_sym = 'e';
function t14() {
    return a14.map(elem => elem.toLowerCase().includes(a14_sym.toLowerCase())).includes(true);
}
...
function t15() {
    ...
    a15_res = a15.filter(item => {
        if (item['pnum'].length == 6 && item['pnum'] != '') {
            return true;
        };
    });
    return a15_res;
